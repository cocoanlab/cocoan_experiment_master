...
while (1) % button
    ...
    if semicircular
        if use_joystick 
            if joy_button(1) && curr_r > radius*.95 % only when the cursur is close to the arc
                break
            elseif joy_button(1)
                joy_button(1) = false;
            end
        elseif use_mouse 
            if button(1) && curr_r > radius*.95
                break
            elseif button(1)
                button(1) = false;
            end
    ...
    if semicircular
        DrawFormattedText(theWindow, rating_types.prompts{i}, 'center', 20, white, [], [], [], 1.2);
    else
        DrawFormattedText(theWindow, rating_types.prompts{i}, 'center', H/2-150, white, [], [], [], 1.2);
    end
